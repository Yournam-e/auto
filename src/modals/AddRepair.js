...
export const AddReapair = ({
  ...
  function addReapair() {
    ...
    if (
      !shortTitle ||
      (shortTitle && shortTitle.replaceAll(" ", "").length < 3)
    ) {
    ...
    axios
      .post(
        `https://showtime.app-dich.com/api/auto/cars/${params.id}/repairs${window.location.search}`,
        newItem
      )
      .then(async function (response) {
        setRepairsInfo([...reapairsInfo, response.data]);
  ...
  useEffect(() => {
    console.log(carData.releaseYear);
  }, []);
